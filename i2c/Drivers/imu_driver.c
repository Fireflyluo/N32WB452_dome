...
BSP_I2C_Device imu_device;
// I2C通信函数实现
static icm42688_err_t i2c_write_reg(uint8_t dev_addr, uint8_t reg_addr, 
                                   const uint8_t *data, uint16_t len) {
    // 实现I2C写寄存器操作

    BSP_I2C_Master_Transmit(&imu_device, reg_addr, (uint8_t *)data, len);
    return ICM42688_OK;
}
...
static icm42688_err_t i2c_read_reg(uint8_t dev_addr, uint8_t reg_addr, 
    ...
    // 实现I2C读寄存器操作
    BSP_I2C_Master_Receive(&imu_device, reg_addr, data, len);
    return ICM42688_OK;
}
...
int main(void) {
    ...
    icm42688_comm_config_t comm_config = {
        ...
        .dev_addr = ICM42688P_I2C_ADDR_0,  // SDO接GND
